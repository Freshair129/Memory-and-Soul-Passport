...
|---|---|---|---|---|---|
| 0.1.4b | 2026-09-14 | beta | TASK-MEMOS-002 stage 1 (API-011 thread memory): `npm run test:security` grew from 45 to 57 cases with the addition of `tests/security/thread-memory-scoping.security.mjs` (12 cases: C-1 read-isolation attack reproductions, W1/W6/W7, and the RKOI post-implementation review items) -- the pre-existing 45 vault-isolation cases this row's "30/30" baseline grew into by WP-14/WP-16/pipeline/promotions work are unchanged and still pass. | working-tree | KIN |
