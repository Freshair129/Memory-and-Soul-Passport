...
1. Refuse if the connection is already inside a transaction -- `PRAGMA foreign_keys` cannot be changed there, so this mode cannot run nested inside one.
...
**A directive that is not on the first line is not a directive -- and is refused, not silently ignored.** If the exact directive text appears anywhere in the file's leading comment block (every line from the top that is blank or a `--` comment, up to the first line that is neither) but not as the file's exact first line -- wrong line, a leading UTF-8 byte-order mark, leading or trailing whitespace, or different casing all count -- the runner throws `migration_directive_misplaced:` naming the file, rather than quietly running the migration on the plain path against a table that likely still holds rows the author meant to protect. A file that does not contain the directive text anywhere is unaffected.

The failures this mode can produce are all the existing startup `SchemaVersionError` (`code = "db_unavailable"`) -- the server refuses to start rather than run against a schema it cannot prove is internally consistent -- distinguished only by message prefix, the same way the checksum-drift and downgrade guards each have their own recognizable prefix: `migration_foreign_key_check_failed:`, `migration_preexisting_foreign_key_violation:`, and `migration_directive_misplaced:`.
