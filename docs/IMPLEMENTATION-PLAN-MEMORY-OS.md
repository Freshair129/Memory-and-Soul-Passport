...
| RSK-MEMOS-09 | **New (RKOI stage-1 code review round 2 on code commit `95629e2`).** A delivery record naming a foreign tenant's `thread_pending_deliveries.receipt_id` answers differently from one naming an unused `receipt_id` — a cross-tenant existence oracle. Low severity (it leaks only that some `receipt_id` exists somewhere, not its content), but real. **Closed by KIN in code commit `d5b518a`**: a colliding foreign-tenant pending `receipt_id` now gets the same generic `conflict` text as any other id collision, proven in `tests/integration/thread-memory.test.mjs` | low / low | Closed; pending RKOI confirmation in the stage-1 code re-review (BL-MEMOS-033) | KIN |
