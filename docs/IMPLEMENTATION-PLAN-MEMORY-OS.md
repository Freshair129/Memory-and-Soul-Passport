...
- [`DESIGN-SESSION-EPISODIC-INSTANCE-MEMORY.md`](DESIGN-SESSION-EPISODIC-INSTANCE-MEMORY.md), now at v0.3.4b after three RKOI NEEDS REVISION rounds (v0.3.0b, v0.3.1b, v0.3.2b) and one APPROVED-with-warnings round (v0.3.3b → v0.3.4b) (BL-MEMOS-011, BL-MEMOS-012); each round reads the shipped stage-1 code (and, from round three on, zuri-ai's own real signer) more directly than the last as the source of truth for wire/schema shapes.
...
| PH-MEMOS-1 | TASK-MEMOS-001 | Decision record and design (current: v0.3.4b) around API-011, including the multi-user/multi-agent model | GATE-MEMOS-1 | in-progress |
...
- **GATE-MEMOS-1**
  - The ADR (current: v0.1.4b) and design (current: v0.3.4b) are merged.
...
- **GATE-MEMOS-4** — every case below runs in `tests/security/thread-erasure.security.mjs` (design §15):
...
- **GATE-MEMOS-6** — every case below runs in `tests/security/consolidation-vault-scoping.security.mjs`, `cross-thread-digest-scoping.security.mjs` and `group-thread-private-context.security.mjs`, plus the extended `erasure-invalidates-retrieval.security.mjs` (design §15):
...
| BL-MEMOS-029 | TASK-MEMOS-002 | W10: typed errors per design v0.3.4b §14 (`validation_failed`, `not_found`, `thread_scope_denied`, `thread_audience_mismatch`, `conflict`, `identity_hmac_unconfigured`, `payload_too_large`, `record_subject_mismatch`, `compaction_lease_conflict`) — `agent_not_current` is a stage-2 code, not shipped in stage 1 | KIN | BL-MEMOS-020, **BL-MEMOS-012** | contract tests assert codes | in-progress |
| BL-MEMOS-030 | TASK-MEMOS-002 | Rename contract API-010 → API-011 (doc + schema) using the branch's actual frozen wire shapes per design v0.3.4b §12.1/§13 (flat epoch/hex grant, snake_case fields, `text`/`exchange_id`/`direction`/`kind`, not the invented camelCase/nested shapes RKOI rejected); client env allowlist gains `MSP_THREAD_SERVICE_KEY`, `MSP_IDENTITY_HMAC_KEY`; client CHANGELOG | KIN | BL-MEMOS-020, **BL-MEMOS-012** | `msp-client-env-allowlist.test.mjs`; keys never journaled or echoed; contract tests match the branch's real field names | in-progress |
