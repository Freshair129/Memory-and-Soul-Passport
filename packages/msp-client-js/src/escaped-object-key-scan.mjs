...
// only caller on the server side (stdio-jsonrpc-server.mjs's
// `rl.on("line", ...)`) had no try/catch around it -- and this client's own
// stdout `data` listener had the identical gap. A single ~40 KB line of
// ~20,000 nested arrays blew the call stack (`RangeError: Maximum call
// stack size exceeded`), uncaught, inside a synchronous event-emitter
// callback -- a crash there would take down the CALLING application, not
// just this client. Fixed two ways, independently, so a defect in one
// layer cannot regress the other into a crash again:
...
//      practical DoS surface for a response this transport already accepts
...
export function containsEscapedObjectKey(raw) {
  try {
    ...
    // this client's own stdout listener) -- refuse the line instead of
