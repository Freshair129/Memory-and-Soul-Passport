// API-011 thread memory (TASK-MEMOS-002): the compaction worker's business
// logic, exercised through the REAL guarded toolRegistry.dispatch (signed
// grants via signThreadRequest) -- unlike thread-memory.test.mjs, which
// uses the unguarded server.threadHandlers map, this file's `call` goes
// through thread-guard.mjs on every request, matching how a real worker
// process would call MSP.
...
async function fixture(run) {
  ...
  const server = createServer({
    ...
    // W1: MSP_TEST_CLOCK=1 lets this fixture's synthetic `time` reach the
    // domain layer. MSP_IDENTITY_HMAC_KEY is required for every thread/
    // journal identity hash (resolveThread, appendMessage, sweep, delivery).
...
describe('authorized durable thread summary worker', () => {
  it('waits for an active answer across idle timeout, then captures the answer in the original session', () => fixture(async ({ server, call, thread, inbound, out, setTime }) => {
    ...
    // RKOI review, item 11: claim never returns protected records.
    expect(job.protectedRecords).toBeUndefined();
  ...
  it('queues fallback before inbound exists, then reconciles it (never deletes it) once, without trusting another scope', () => fixture(async ({ server, call, thread }) => {
    ...
    expect((await call('msp_thread_delivery_record', receipt)).status).toBe('PENDING_INBOUND');
    expect(server.db.prepare("SELECT COUNT(*) AS n FROM thread_pending_deliveries WHERE reconcile_state='pending'").get().n).toBe(1);
    ...
    // RKOI review, item 6: reconciled, never deleted -- the row still
    // exists, just no longer pending.
    expect(server.db.prepare("SELECT COUNT(*) AS n FROM thread_pending_deliveries WHERE reconcile_state='pending'").get().n).toBe(0);
    expect(server.db.prepare("SELECT COUNT(*) AS n FROM thread_pending_deliveries WHERE reconcile_state='reconciled'").get().n).toBe(1);
