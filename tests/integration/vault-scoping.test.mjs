...
describe("WP-14 AC-01: migration 0003_vault_scoping.sql", () => {
  it("applies idempotently against a fresh database (packaged migrations, no error, no duplicate application)", () => {
    const db = freshDb();
    const first = runMigrations(db, migrationsDir);
    expect(first.appliedCount).toBe(14);
    expect(first.currentVersion).toBe(14);
