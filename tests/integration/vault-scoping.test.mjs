...
describe("WP-14 AC-01: migration 0003_vault_scoping.sql", () => {
  it("applies idempotently against a fresh database (ten migrations, no error, no duplicate application)", () => {
    const db = freshDb();
    const first = runMigrations(db, migrationsDir);
    expect(first.appliedCount).toBe(10);
    expect(first.currentVersion).toBe(10);
