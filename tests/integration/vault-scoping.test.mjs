...
describe("WP-14 AC-01: migration 0003_vault_scoping.sql", () => {
  it("applies idempotently against a fresh database (twelve migrations, no error, no duplicate application)", () => {
    const db = freshDb();
    const first = runMigrations(db, migrationsDir);
    expect(first.appliedCount).toBe(12);
    expect(first.currentVersion).toBe(12);
